        ...
        <div className="space-y-2">
          {items.map((item, i) => (
            <motion.div
              ...
              className={`flex items-start sm:items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl border border-border/40 bg-card/30 border-l-2 ${leftBorderStyles[item.priority] || ""}`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                  ...
                  {item.callerPhone && (
                    <span className="text-[10px] text-muted-foreground/60 font-mono hidden sm:inline">{item.callerPhone}</span>
                  )}
                  <Badge className={`text-[10px] h-4 px-1.5 rounded-full border ${priorityStyles[item.priority] || ""}`}>
                    {priorityLabels[item.priority] || item.priority}
                  </Badge>
                  <Badge variant="outline" className="text-[10px] h-4 px-1.5 rounded-full hidden sm:inline-flex">
              ...
              <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                <p className="text-[10px] sm:text-xs text-muted-foreground">{item.timeLabel}</p>
                <Button
                  variant="ghost"
                  size="icon"
