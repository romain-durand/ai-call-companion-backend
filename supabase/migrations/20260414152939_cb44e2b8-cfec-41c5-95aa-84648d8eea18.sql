...
-- 1. Create new enum
CREATE TYPE public.call_behavior_new AS ENUM ('take_message', 'transfer', 'ask_user', 'book_appointment', 'block');

-- 2. Add temporary column
ALTER TABLE public.call_handling_rules ADD COLUMN behavior_new public.call_behavior_new;

-- 3. Migrate existing data
UPDATE public.call_handling_rules SET behavior_new = CASE
  WHEN behavior::text = 'answer_and_take_message' THEN 'take_message'::call_behavior_new
  WHEN behavior::text = 'answer_and_transfer' THEN 'transfer'::call_behavior_new
  WHEN behavior::text = 'answer_and_escalate' THEN 'ask_user'::call_behavior_new
  WHEN behavior::text = 'answer_and_book' THEN 'book_appointment'::call_behavior_new
  WHEN behavior::text = 'answer_only' THEN 'take_message'::call_behavior_new
  WHEN behavior::text = 'voicemail' THEN 'take_message'::call_behavior_new
  WHEN behavior::text = 'block' THEN 'block'::call_behavior_new
  ELSE 'take_message'::call_behavior_new
END;

-- 4. Drop old column and rename
ALTER TABLE public.call_handling_rules DROP COLUMN behavior;
ALTER TABLE public.call_handling_rules RENAME COLUMN behavior_new TO behavior;
ALTER TABLE public.call_handling_rules ALTER COLUMN behavior SET NOT NULL;
ALTER TABLE public.call_handling_rules ALTER COLUMN behavior SET DEFAULT 'take_message'::call_behavior_new;

-- 5. Drop old enum and rename new one
DROP TYPE public.call_behavior;
ALTER TYPE public.call_behavior_new RENAME TO call_behavior;

-- 6. Remove obsolete columns
ALTER TABLE public.call_handling_rules DROP COLUMN IF EXISTS callback_allowed;
ALTER TABLE public.call_handling_rules DROP COLUMN IF EXISTS booking_allowed;
ALTER TABLE public.call_handling_rules DROP COLUMN IF EXISTS escalation_allowed;
ALTER TABLE public.call_handling_rules DROP COLUMN IF EXISTS force_escalation;

-- 7. Update handle_new_user function with new behaviors
...
BEGIN
  ...
  INSERT INTO public.phone_numbers (account_id, e164_number, label, provider, country_code, status, verification_status, ownership_type, is_default_outbound, capabilities)
  VALUES (_account_id, '+33939244855', 'Numéro de collecte', 'twilio', 'FR', 'active', 'verified', 'rented', false, '{"voice": true, "sms": false}'::jsonb);
  ...
  FOR _m IN 1..array_length(_modes, 1) LOOP
    ...
    FOR _i IN 1..array_length(_group_slugs, 1) LOOP
      _behavior := CASE
        WHEN _modes[_m][2] = 'work' THEN
          CASE _group_slugs[_i]
            WHEN 'family' THEN 'transfer'::call_behavior
            WHEN 'vip' THEN 'transfer'::call_behavior
            WHEN 'work_group' THEN 'book_appointment'::call_behavior
            WHEN 'blocked' THEN 'block'::call_behavior
            ELSE 'take_message'::call_behavior
          END
        WHEN _modes[_m][2] = 'personal' THEN
          CASE _group_slugs[_i]
            WHEN 'family' THEN 'transfer'::call_behavior
            WHEN 'friends' THEN 'transfer'::call_behavior
            WHEN 'blocked' THEN 'block'::call_behavior
            ELSE 'take_message'::call_behavior
          END
        WHEN _modes[_m][2] = 'night' THEN
          CASE _group_slugs[_i]
            WHEN 'family' THEN 'ask_user'::call_behavior
            WHEN 'blocked' THEN 'block'::call_behavior
            ELSE 'take_message'::call_behavior
          END
      ...
      INSERT INTO public.call_handling_rules (
        ...
        summary_required, priority_rank
      ) VALUES (
        _account_id, _mode_id, _group_ids[_i], _behavior,
        true, _default_groups[_i][5]::int
      );
    END LOOP;
  END LOOP;
